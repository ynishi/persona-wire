...
use crate::infrastructure::adapter::{Adapter, Cursor, Pageable, NON_PAGEABLE_MAX_HINT};
...
/// Wire-layer pagination-aware fetch driver (Layer 2 of GH #1, follow-up to
/// the `Pageable` trait + `Cursor` enum added in Layer 1).
///
/// Extracts the `?limit=` query param (a convention every current adapter
/// shares) and routes into one of 3 branches:
///
/// 1. **Pageable + `limit > max_page_size`** — drives
///    [`drive_pageable_loop`] through the adapter's [`Pageable::fetch_page`]
///    until enough items are collected or the upstream signals
///    end-of-data.
/// 2. **Non-Pageable + `limit > NON_PAGEABLE_MAX_HINT`** — the adapter
///    cannot fulfill the request; emits a `tracing::warn!` naming the
///    adapter's scheme, requested limit, and the heuristic threshold, then
///    falls through to a plain (capped) `Adapter::fetch`.
/// 3. **Default** (`limit` absent/unparseable, or `limit` within bounds) —
///    plain `Adapter::fetch`, unchanged behavior for all current callers.
async fn fetch_with_pagination_awareness(
    adapter: &Arc<dyn Adapter>,
    uri: &WireUri,
) -> WireResult<serde_json::Value> {
    let Some(limit) = uri.query_get("limit").and_then(|s| s.parse::<usize>().ok()) else {
        return adapter.fetch(uri).await;
    };
    ...
    match adapter.as_pageable() {
        ...
        None if limit > NON_PAGEABLE_MAX_HINT => {
            let max_page_size = NON_PAGEABLE_MAX_HINT;
            tracing::warn!(
                target: "persona_wire::pagination",
                scheme = adapter.scheme(),
                limit,
                max_page_size,
                "adapter does not support pagination; requested limit exceeds max_page_size, returning capped result"
            );
            adapter.fetch(uri).await
        }
        None => adapter.fetch(uri).await,
    }
}

/// Drives a [`Pageable::fetch_page`] loop, threading the returned `Cursor`
/// until either `limit` items are accumulated or the upstream signals
...
async fn drive_pageable_loop(
    ...
    loop {
        let (page_items, next_cursor) = pageable.fetch_page(uri, cursor).await?;
...
async fn render_collected_slot_async(
    ...
    let fetched = match registry.route(&c.source_uri) {
        Ok((adapter, uri)) => match fetch_with_pagination_awareness(&adapter, &uri).await {
            Ok(v) => v,
            Err(e) => {
                warnings.push(format!(
...
mod tests {
    ...
    // ---- fetch_with_pagination_awareness / drive_pageable_loop (Layer 2, GH #1) ----

    use std::sync::atomic::{AtomicUsize, Ordering};
    use std::sync::Mutex;

    /// Adapter that never implements `Pageable` (default `as_pageable` ==
    /// `None`), used to exercise the non-Pageable + over-limit WARN branch.
    struct MockNonPageableAdapter;

    #[async_trait::async_trait]
    impl Adapter for MockNonPageableAdapter {
        fn scheme(&self) -> &'static str {
            "mocknp"
        }

        async fn fetch(&self, _uri: &WireUri) -> WireResult<serde_json::Value> {
            Ok(json!({"scheme": "mocknp", "count": 1, "items": [{"id": 1}]}))
        }
    }

    /// Pageable mock whose pages are scripted to overshoot the requested
    /// `limit` on the 2nd page — verifies the loop stops as soon as
    /// `items.len() >= limit` (without waiting for `next_cursor == None`)
    /// and that the result is truncated to exactly `limit`.
    struct LoopUntilLimitAdapter {
        call_count: AtomicUsize,
    }

    impl LoopUntilLimitAdapter {
        fn new() -> Self {
            Self {
    ...
    #[async_trait::async_trait]
    impl Adapter for LoopUntilLimitAdapter {
        fn scheme(&self) -> &'static str {
            "mockpage"
        }

        async fn fetch(&self, _uri: &WireUri) -> WireResult<serde_json::Value> {
            Ok(json!({"scheme": "mockpage", "count": 0, "items": []}))
        }
    ...
    #[async_trait::async_trait]
    impl Pageable for LoopUntilLimitAdapter {
        ...
        async fn fetch_page(
            ...
            match cursor {
                None => Ok((
                    ...
                    Some(Cursor::PageNumber(2)),
                )),
                Some(Cursor::PageNumber(2)) => Ok((
                    vec![json!({"id": 3}), json!({"id": 4})],
                    Some(Cursor::PageNumber(3)),
                )),
                other => Err(WireError::Storage(format!(
                    "LoopUntilLimitAdapter: unexpected cursor: {other:?}"
                ))),
            }
        }
    }

    /// Pageable mock whose upstream runs out of data (`next_cursor = None`)
    /// before the requested `limit` is reached — verifies the loop
    /// terminates gracefully and returns whatever was actually fetched.
    struct ShortPageableAdapter;

    #[async_trait::async_trait]
    impl Adapter for ShortPageableAdapter {
        fn scheme(&self) -> &'static str {
            "mockshort"
        }

        async fn fetch(&self, _uri: &WireUri) -> WireResult<serde_json::Value> {
            Ok(json!({"scheme": "mockshort", "count": 0, "items": []}))
        }
    ...
    #[async_trait::async_trait]
    impl Pageable for ShortPageableAdapter {
        fn max_page_size(&self) -> usize {
            2
        }
        ...
        async fn fetch_page(
            ...
            match cursor {
                ...
                Some(Cursor::PageNumber(2)) => Ok((vec![json!({"id": 3})], None)),
                other => Err(WireError::Storage(format!(
                    "ShortPageableAdapter: unexpected cursor: {other:?}"
                ))),
            }
        }
    }

    /// `tracing` capture harness — a `MakeWriter` that appends every
    /// formatted event into a shared buffer so tests can assert on WARN
    /// message content without an extra `tracing-test`-style crate.
    #[derive(Clone)]
    ...
    #[tokio::test]
    async fn wire_warns_on_non_pageable_over_limit() {
        ...
        let adapter: Arc<dyn Adapter> = Arc::new(MockNonPageableAdapter);
        let uri = WireUri::parse("mock://items?limit=150").unwrap();
        let result = fetch_with_pagination_awareness(&adapter, &uri)
            .await
            .unwrap();
        ...
        assert_eq!(
            result["count"], 1,
            "non-Pageable path returns capped fetch result unchanged"
        );
        let output = String::from_utf8(buf.lock().unwrap().clone()).unwrap();
        assert!(
            output.contains("adapter does not support pagination"),
            "expected WARN message in captured tracing output, got: {output}"
        );
        assert!(
            output.contains("mocknp"),
            "expected adapter scheme in captured tracing output, got: {output}"
        );
    }

    #[tokio::test]
    async fn wire_drives_pageable_loop_up_to_limit() {
        let concrete = Arc::new(LoopUntilLimitAdapter::new());
        let adapter: Arc<dyn Adapter> = concrete.clone();
        let uri = WireUri::parse("mock://items?limit=3").unwrap();
        ...
        assert_eq!(
            concrete.calls(),
            2,
            "loop must stop as soon as accumulated items satisfy limit (2 fetch_page calls)"
        );
    }

    #[tokio::test]
    async fn wire_pageable_stops_on_none_cursor() {
        let adapter: Arc<dyn Adapter> = Arc::new(ShortPageableAdapter);
        // limit (10) exceeds max_page_size (2) and exceeds total available
        // items (3) — the upstream runs out before `limit` is reached.
        let uri = WireUri::parse("mock://items?limit=10").unwrap();
