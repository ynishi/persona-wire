...
| TC-013 | wire_render | 登録済 NamedProjection を name 指定で 1 つだけ呼ぶ、 dangling spec_ref error / 不在 projection_ref error | ✓ test + 手動 | wire_render land |
...
## TC-013: wire_render (registered NamedProjection 個別呼出)

- **目的**: 登録済 NamedProjection を name 指定で 1 つだけ render する semantic を verify。 wire_init (全 projection 一括) の counterpart
- **前提**: spec + projection register 済 (例: `_active` projection が `active_personas` spec 参照)
- **手順 (test 内蔵 2 case、 `tests/p2b_e2e.rs::wire_render_*`)**:
  1. `wire_render_evaluates_registered_projection_by_name`: `_active` projection を name 指定で呼出、 戻り値が `{name, target_form, rendered}`、 rendered に matched node id が含まれる
  2. `wire_render_errors_on_unknown_projection_and_dangling_spec`: (a) 不在 projection_ref → `NotFound("projection: ...")`、 (b) dangling spec_ref (= projection は存在するが参照 spec が不在) → `NotFound("spec_ref (dangling): ...")`

### CLI form (手動 smoke)

```bash
# 登録済 projection の個別 render
persona-wire render --projection-ref _active
# → {"name": "_active", "target_form": "prompt", "rendered": "Active personas (3): p1, p2, p3"}
```

### MCP form

```
mcp__persona-wire__wire_render(projection_ref: "_active")
```
...
```json
{
  ...
  "target_form":  "prompt" | "markdown" | "json" | "ascii",
  "rendered":     "<template-evaluated string>"
}
```

### Semantic 注意

- **wire_init との関係**: wire_init は全 NamedProjection を一括 render、 wire_render は **1 つだけ name 指定**。 用途 = 特定 projection を即座に欲しい (例: `_active` だけ inject)、 全 context bundle は不要
- **ad-hoc inline (= spec + template + target_form を直渡し、 register せず 1 回 render)**: 別 surface carry (wire_query 同型の inline 経路拡張、 別 turn 着手 trigger)
- **dangling spec_ref**: wire_init では warn にして他 projection の render を続けるが、 wire_render は単発 = error として return (caller が個別に判断)
...
- 2026-06-15: `wire_render` land — 登録済 NamedProjection を name 指定で個別 render (wire_init の counterpart)、 戻り値 `{name, target_form, rendered}`、 dangling spec_ref / 不在 projection_ref は `NotFound` error 単発 return、 CLI subcommand + MCP Tool + integration test 2 件 (`tests/p2b_e2e.rs` に追加)、 cargo test 67 PASS (49 + 3 + 3 + 6 + 3 + 3)、 TC-013 起こし。 ad-hoc inline (spec + template + target_form 直渡し) は別 surface carry
